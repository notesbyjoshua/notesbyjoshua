...
| --- | --- |
| Central idea | Summarize the passage's main claim or development |
| Detail | Locate and accurately paraphrase stated information |
| Inference | Take one small, supported step beyond what is stated |
...
:::strategy
1. If a passage mentions a species (e.g. Tyrannosaurus Rex), always replace it with something simpler, like "species A."
2. Rephrase jargony sentences to make it simpler/understandable. For example, "The researchers posit that the phenomenon is attributable to a confluence of environmental and genetic factors" can be simplified to "Researchers think the phenomenon is caused by both environmental and genetic factors."
3. Circle the main subject and underline the main verb in each sentence. Ignore long names, dates, parenthetical definitions, and examples until you understand who did what.
4. Mark logic words such as *however*, *because*, *although*, *therefore*, and *for example*. Replace them mentally with simple labels such as *contrast*, *cause*, *concession*, *result*, and *example*.
5. Assign short labels to groups and variables. For example, turn “participants exposed to the multisensory intervention” into “Group A” and “phonemic discrimination accuracy” into “accuracy.”
6. After each sentence, add its job to a short mental outline: *old idea*, *new evidence*, *result*, or *conclusion*. Do not try to memorize every detail.
7. Summarize the entire passage in one plain sentence before evaluating the choices. Return to exact names or numbers only when a choice requires them.
:::

:::tip
When two choices seem logically correct, ask which one answers the exact question with the fewest changes to the passage. Compare the choices phrase by phrase: subject, action, comparison, cause, and degree of certainty. Eliminate a choice if even one important phrase is too broad, too narrow, reversed, or unsupported. A choice can be true according to the passage and still be wrong because it describes background information instead of the requested conclusion.
:::
...
4. Reject choices that contradict what is stated in the passage or describe only one example or piece of background information.
:::
...
:::tip
If two central-idea choices are accurate, prefer the one that includes the passage's main turn and conclusion. A choice about the opening theory or a single result may be true but too narrow; a choice that adds a universal claim may be too broad.
:::
...
:::tip
For a detail-question tie, locate the smallest part of the passage that answers the question and demand a direct match. A choice supported somewhere else in the passage is still wrong if it answers a different detail. Be especially strict about qualifiers such as *some*, *often*, *may*, and *under the tested conditions*.
:::
...
:::tip
If two inference choices are possible, count the assumptions each requires. Prefer the choice that restates the measured relationship with cautious language such as *suggests*, *may*, or *was associated with*. Reject a choice that changes the outcome measured, claims a cause from an association, or extends the result to a new population or condition.
:::
...
:::tip
When two findings support a claim generally, choose the one that distinguishes the claim from its closest competing explanation. The best evidence often holds one variable constant while changing another. Related evidence is weaker if it supports both explanations equally well.
:::
...
:::mistakes
- selecting the correct trend for the wrong group or year,
- confusing a percentage with a percentage-point difference,
- claiming causation from correlation,
- using *always* when the graph shows only a general trend,
- ignoring the units.
:::
...
:::checklist
- Did I read the correct row, bar, line, or category?
- Did I compare the requested groups and time periods?
- Did I use the graph's units?
- Does the choice describe an association, or does it incorrectly claim causation?
- Does the numerical evidence actually support the conclusion in the stem?
:::

:::tip
When two quantitative choices quote correct numbers, check what comparison the claim actually requires. A value can be copied accurately but still be irrelevant. For a claim about an effect, compare treated with untreated values within each group; for a claim about groups, compare the groups under the same condition.
:::
